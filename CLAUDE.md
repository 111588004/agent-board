...
**The different port does NOT mean different data.** `db.js`'s `baseDir` defaults to `~/.agent-board` regardless of `source: "dev"|"npm"` — port only picks which HTTP endpoint you're talking to, not which SQLite files get opened. Running `npm start` (dev, :4316) and the global `agent-board` (npm, :4317) at the same time means **two separate Node processes writing to the exact same `tasks.db` files**, which violates this project's own single-writer-process assumption (see "The REST API is the single source of truth" above) — nothing currently detects or prevents it. If you're testing dev-checkout changes and want to avoid touching real board data, give the dev server its own root: `AGENT_BOARD_DIR=~/.agent-board-dev PORT=4316 npm start`, and point any CLI/agent testing against it at the same URL (`AGENT_BOARD_URL=http://localhost:4316 agent-board ...` — the DB root is a server-side setting, so the CLI side only needs `AGENT_BOARD_URL`, not `AGENT_BOARD_DIR`).
