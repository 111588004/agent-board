...
## v0.2.2 round — project CRUD, description editor, task CRUD (manual verification, not agent-driven)

Not scripted scenarios like above — ad hoc manual checks run directly against the dev server (`:4316`) while building each feature, using the `default` workspace's `agent-board` project (`AB-*` tickets). Recorded here so a later session can tell what's already been exercised vs. what's still unverified.

- **Project rename/delete** (REST + CLI + MCP + web UI) — renamed/deleted throwaway projects (`TmpTest`/`TT`, `TestProj`/`ZZ`) via all four surfaces; confirmed rename cascades to existing tasks' `project`/`projectPrefix`, and delete refuses when the project still has tasks (tested by creating then deleting a task first).
- **New-project dialog** — tested the Name/Prefix two-column dialog with a multi-word name ("Design Tools" → suggests "DT"), a single word ("Inbox" → suggests "IN", not just "I"), a CJK-only name ("吃飯大師" → no suggestion, forces manual prefix entry, confirmed the Create button unblocks once typed), and a deliberate prefix collision with the existing `AB` project (confirmed the 409 "already in use" error surfaces inline in the dialog, not a silent failure).
- **Hand-rolled date picker** (`DueDateField`/`CalendarGrid` in `web/src/App.jsx`) — set/cleared a due date on `AB-1`, verified month navigation (Sept → Oct, no stray "today" ring carried over), verified the stored value round-trips as plain ISO `YYYY-MM-DD` via a direct API check.
- **New task defaults** — confirmed Project no longer silently defaults to the alphabetically-first project (forces an explicit pick unless a project filter is active), Agent defaults to `null`/"Unassigned" (not `"claude"`), Priority still defaults to `"med"`.
- **Description editor** (`renderMarkdown` + toolbar) — `AB-4` ("Test: image + code + notes from local file") is a real, still-existing ticket exercising: H1/H2/H3, a bare `https://` URL autolinking without `[text](url)` syntax, a bullet list, a numbered list, and an image — specifically this repo's own `web/public/favicon.svg`, read from disk and embedded as a base64 data URI (chosen over the user's real screenshots to avoid publishing unreviewed personal content). Also confirmed fenced ` ``` ` code blocks are *not* supported (renders literally) — only single-backtick inline code — and that clicking/dragging to select text in the rendered view no longer hijacks into edit mode (only the hover-reveal pencil button does).
- **Task CRUD completeness** (CLI + MCP) — `AB-5` was created twice as a throwaway: once via `agent-board create` with every new flag (`--due-date`/`--worktree`/`--branch`/`--link`/`--notes`) and deleted via `agent-board delete`; once via a raw MCP `tools/call` for `delete_task`. MCP `update_task` was checked against `AB-2` (changed `priority`/`branch`, confirmed the auto-history-log line got appended, then reverted). Neither `AB-5` instance nor the test mutations on `AB-2`'s priority/branch were left in place.
- **Dev/npm port split** — confirmed `:4316` (dev) and `:4317` (npm) can run concurrently, each self-reporting the correct `source` via `/api/meta`; not re-tested after every subsequent change since the mechanism (source-based default in `server.js`) hasn't been touched since.

**Still on the board, not cleaned up:** `AB-4` (the image/code/notes test ticket) is intentionally left in place as a live example — ask before deleting it if a future session doesn't recognize why it's there.
