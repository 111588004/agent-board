...
import { db, createTask, appendNote } from "../db.js";
...
router.get("/", (req, res) => {
  ...
  res.json(db.prepare(`SELECT * FROM tasks ${where} ORDER BY createdAt`).all(...params));
});
...
router.patch("/:id", (req, res) => {
  ...
  if (req.body.note !== undefined) {
    row = appendNote(req.params.id, req.body.note, req.body.agent);
  }
  res.json(row);
});
